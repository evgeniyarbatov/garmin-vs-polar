...
def get_db_conn():
    return psycopg2.connect(
        host="localhost", database="gpx", user="root", password="root"
    )
...
def init_postgis(device_table_name, gpx_filepath):
    query = f"""
    ...
    try:
        ...
        for point in gpx_points:
            ...
            with conn.cursor() as cur:
                cur.execute(
                    f"""
              INSERT INTO {device_table_name} (time, location)
...
def get_location_error(device_table_name):
    query = f"""
...
"""
    ...
    df = pd.DataFrame(
        rows, columns=["time", "latitude", "longitude", "distance_to_route"]
    )
...
def crop_route(df, start_location, stop_location, start_point_count):
    df["start_distance"] = df.apply(
        lambda row: geodesic(
            (row["latitude"], row["longitude"]), start_location
        ).meters,
        axis=1,
    )
    df["stop_distance"] = df.apply(
        lambda row: geodesic((row["latitude"], row["longitude"]), stop_location).meters,
        axis=1,
    )
...
def get_total_distance(df):
    ...
    df["distance"] = df.apply(
        lambda x: geodesic(
            (x["prev_latitude"], x["prev_longitude"]), (x["latitude"], x["longitude"])
        ).meters,
        axis=1,
    )
